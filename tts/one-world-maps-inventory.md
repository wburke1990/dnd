...
> `d8cc23` `00c35d` `85ff11` `9e7345` `3695ec` `ca7a72` `e0559d` `824863`
> `dd414a` `d2b323` `d18d1c` `189d29`
...
|-----|------|--------|-------|
| **Haagen** | 340cba | ✅ **done — Haagen** | **Not from the OW library** — a raw Workshop mod the user loaded into slot 25 (a French board game: houses, statues, a fountain, a monument), packed into an OWx bag by `pack-ow-map` on 8/28. 40 pieces off the table surface; the table model, its cabinets, the hand zones, seven backdrop boards, the merchant chip and the two player aids were left behind. **Zero dead URLs of 35 probed.** Its pieces sat centred on (13.71, −0.76), which is why an in-game resizer looked broken; the packer recentres them. Imported at **`--scale 1.5`** with the Hub floor left at its default 25 — the map is sized to the table, not the floor to the map, after three failed attempts the other way round ([oneworld.md](../docs/oneworld.md)). **The map brings its own floor**, so the Hub's painted floor is set to **SBx_Wood Table**'s image rather than a ground texture — cobbles under it clashed. **Haagen is its own town** — it is not a stand-in for Copaa or anywhere else, and Small town still holds the Copaa job. |
| **The Mumbling Rat** | 90bd67 | ❌ not usable — removed from staging | Imported 8/28 as an **[Aar](../world/suartleheim-eet/raand-copaa/lore/the-tally-house-and-the-guest-house.md) Tally House** candidate and deleted the same day. The prune took **126 pieces, 34 dead of 67 URLs**, and what survived is minis and blankets — the walls rode dead hosts. The sparse-import warning was right. Do not re-import. |
| **Nonspecific Inn** | b2a074 | ❌ not usable — removed from staging | Imported 8/28 and deleted 9/1. The prune took **159 pieces, 78 dead of 173 URLs**, and what survived has no walls and no floor texture — its floor was the known dead black placeholder (`coolbackgrounds.io`). **Worth one thing:** a set of townsfolk minis that appear in no other opened map, which have been lifted out and kept. Do not re-import for the building. |
| **BlueWater Inn** | 1fd7e3 | ✅ **done — the Aar Tally House** | Imported 8/28, best of the four tavern candidates, aligned 9/1. Light prune: **34 pieces removed, 28 dead of 139 URLs**. Default vBase 25, no plate of its own. **Its floor image is a drawn battlemap of the inn's ground floor, so the 3D pieces have to stand on the printed rooms** — which is what all the adjustment below was for. Three fixes, all via `adjust-ow-map` with no re-import: rotated **180°**; floor image **padded** and re-hosted on TTS Cloud, because OW clipped the edges even at its native 1600 × 945 ([oneworld.md](../docs/oneworld.md)); and scaled to **1.10× its imported size**, final span **78.6 × 34.2**. **One 5 ft square is 12.8 units of span** on this map — the number to use if it ever needs re-aligning. |
| **Brick bar** | b03586 | 🔁 duplicate — removed from staging | Imported 8/28 and deleted the same day: **a Brick bar with better walls and floor already exists in the save**, so this one was redundant. The import itself was clean (**1 dead URL, 0 pieces removed**), but 83 wall textures came in blanked off that one dead URL, which is what the existing copy has already had fixed. Check for an existing copy before re-importing. |
...
| **Valience Farmhouse** | c3aa13 | ✅ **in staging — the Thinker's hut** | A primitive rural farmhouse — too rural for the mill, banked, then **re-imported into staging 9/15** as the hut on the hilltop in [the Last Thinker](../world/island-of-the-thinkers/encounters/the-last-thinker.md). No plate detected → default vBase 25 (the default fits). **Heavy prune both times: 23 dead of 55 URLs, 23 of its 58 pieces removed** — eyeball it for sparseness before building the scene on it. Its tumblr floor image is live. The stashed clean bag at `~/Library/Tabletop Simulator/map-stash/farmhouse_clean_c3aa13.json` **is not on this machine**, so the re-import came from donor `c3aa13` in TS_Save_22 and was pruned again. New SBx GUID `6febaf`. **A tree prop has been lifted out of it** for the Thinker's hilltop. |
...
| **Mithral Mine** | fdc25b | ❌ doesn't fit the OW floor — great content | Carved numbered square rooms, numbered cave rooms, an animated centaur archer, Cerberus torn apart at the entrance, a passage to a big body of water. Clean import (0 dead of 55). The floor can't be made to line up: the map is ~90x102, its walkable floor is a strip of square plates off to one side (all at Z≈0) while the terrain sprawls ~70 units past it in -Z — no single OW floor plate covers or centers it. (This is NOT the thin-bar plate-fit bug fixed 2026-07 — Mithral's tiles are square; it's the map's geometry.) **Removed from staging.** Content is the best of the mine candidates, but for the mines use **Canyon Cave** (55ed53, already flush). |
...
| **Dwarven Cavern** | ff391e | ⚠️ re-importable — tool fixed | Carved tunnels in rock, big treasure chests, dead dwarf minis, one pool of water. The floor/mesh offset was the plate-fit grabbing an 11x1 wall beam as the "floor" and recentering the map on it — **fixed 2026-07** (`import_ow_map`: a plate must be broad in both axes, else fall back to default). It now imports at default vBase 25 with no recenter, and its geometry sits near the origin, so a fresh import should line up. **Removed from staging** (lower priority than Middle Dwarf Town for Haals); re-import clean from donor ff391e if wanted. 65 pieces pruned (27 dead of 92). DeviantArt floor image (decay-prone). |
...
| **Small town** | 189d29 | ✅ leading Copaa candidate | A small town with a temple at the center and a statue of a sword-bearing warrior; small buildings and a smithy; detailed minis. Near-clean import (1 dead of 36). Its floor image was dead and rocky (wrong for a green setting), so **the SBx floor image was swapped** to Forest Clearing's grass floor (Steam-hosted). Its pond needed a **180° rotation** to sit right against the town pieces; the rotated image is hosted on **TTS Cloud** at `steamusercontent-a.akamaihd.net/ugc/11935844772624656728/4538301478A9CAAB48F9148595A7F0FBAFCED99A/` (GitHub can't host campaign images — see [tts-asset-debug.md](../docs/tts-asset-debug.md)). Re-apply that URL to the SBx floor if the map is re-imported. In staging as the likely Copaa map, and its grass floor is the ground texture for the [Island of the Thinkers](../world/island-of-the-thinkers/encounters/the-last-thinker.md) hilltop. |
| **Hill town tavern** | d18d1c | ⚠️ ok — dropped | A one-room tavern in the bend of a blocky river on a hill of blocky platforms; green, a few trees. Reads as unrealistic. 29 pieces pruned (7 dead); floor image live. **Removed from staging** — no setting for it, and not good enough to bank. |
| **Rat City** | e0559d | ⚠️ token source — dropped | The floor image is a "Last Supper" painting with rats (the token's `Council_of_13` image); a city sits on top of it, incongruous. Two building meshes are dead — a rope bridge hangs in the air with nothing at its ends. Good source of tokens/minis if we build our own map. **Removed from staging.** |
| **The Docks** | 824863 | ❌ not usable | Stub map — only ~12 asset pieces and 10 were dead, so prune stripped it to nothing. Same failure as Port city. **Removed from staging.** |
| **Outpost 19** | dd414a | ⚠️ not chosen — dropped | A canyon carved by a winding river, two entrances; stacked logs and a few carts. Incongruous: flat tokens of emus behind a fence. Plate-fit (vBase 18.06); 5 pieces pruned (4 dead). **Removed from staging.** |
| **Odnarb Point** | d2b323 | ❌ not usable — steal houses | A tower with a few houses and a small water tower on raised ground by the water, two rowboats moored below. Floor image dead (errors before Build) and would need to show the riverbank; shrubs/moss lost their textures (white). 33 pieces pruned (15 dead), 75 blanked. **Removed from staging** — usable only to lift houses/props. |
...
| **Druid's Circle** | f0152d | ✅ **good — keep, unused so far** | Imported and built 9/15. **A ring of standing stones around a druid camp: a fire, and a tree with a skull shape in it.** 86 pieces, 11 removed (4 dead of 46 URLs). No plate → default vBase 25. Steam-UGC floor, live, and the floor is a drawn battlemap of the stone ring rather than a ground texture. Looked at for the [Island of the Thinkers](../world/island-of-the-thinkers/encounters/the-last-thinker.md) and passed over for Grandfather Tree — **in staging, and worth a scene of its own.** New SBx `78baa4`. |
| **Grandfather Tree** | 9fcef4 | ✅ **winner — the Thinker's hilltop** | Imported and built 9/15, and chosen for [the Last Thinker](../world/island-of-the-thinkers/encounters/the-last-thinker.md). Only 4 objects in the bag, so it is a tree rather than a map — which is what that scene wants. Cleanest import so far: 1 dead of 6 URLs, nothing removed, plate-fit vBase 22.0. **Its own imgur floor image is a printed map illustration**, title card and "CROWN / OUTER RING / INNER RING" labels and all, so the SBx paint was swapped for **Small town's grass with the pond painted out**. The image is hosted on **TTS Cloud** at `steamusercontent-a.akamaihd.net/ugc/10968524347360096847/71EE7356D9FA86F66C0767655A5E1C5972A217CC/` (swapped in 9/22). Re-apply that URL to the SBx floor if the map is re-imported. New SBx `b739d8`. |
| **Low Hill** | 6e9f87 | 🔖 **reserved — a camp, not the Thinker's hill** | Imported and built 9/15. **A camp: trees, tents, and the gear that goes with them** — worth keeping for a camp scene, and not used for the [Island of the Thinkers](../world/island-of-the-thinkers/encounters/the-last-thinker.md). **30 of its 59 pieces were removed** (8 dead of 16 URLs), so what stands is half the donor map. Its Google-Sites floor image was **dead** and the map has no plate of its own, so the SBx paint was swapped to Small town's grass image (`steamusercontent-a.akamaihd.net/ugc/11935844772624656728/4538301478A9CAAB48F9148595A7F0FBAFCED99A/`, live, 800 KB), which has a pond in it. 47 GUID collisions remapped on import. New SBx `401161`. |
