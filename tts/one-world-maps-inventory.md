...
|-----|------|--------|-------|
| **Haagen** | c4cd68 | ⏳ in staging — not yet eyeballed | **Not from the OW library** — a raw Workshop mod the user loaded into slot 25 (a French board game: houses, statues, a fountain, a monument), packed into an OWx bag by `pack-ow-map` on 8/28. 40 pieces off the table surface; the table model, its cabinets, the hand zones, seven backdrop boards, the merchant chip and the two player aids were left behind. **Zero dead URLs of 35 probed** — the cleanest import yet. Its pieces sat centred on (13.71, -0.76), which is why an in-game resizer looked broken; the packer recentred them. No floor plate of its own, so vBase is forced: **10.5** against a 41.8 × 27.7 span. It went in at 48 first, which put the floor over the DM area and part of the player areas — vBase scales a plate ~4 units across, so it is the span over 4.1, not the span ([oneworld.md](../docs/oneworld.md)). Floor image is **SBx_Cobble**'s, already live in staging. Candidate for **Copaa**. |
