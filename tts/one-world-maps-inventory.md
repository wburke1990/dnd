...
| **Druid's Circle** | f0152d | ⚠️ **imported 9/15 — not yet eyeballed** | Imported into staging 9/15 as an [Island of the Thinkers](../world/suartleheim-eet/brauron/encounters/the-last-thinker.md) candidate. 86 pieces, of which 11 were removed (4 dead of 46 URLs) — a light prune. No plate → default vBase 25. Steam-UGC floor image, live. New SBx `78baa4`. **Build it and judge it** — the name is still only a name. |
| **Grandfather Tree** | 9fcef4 | ⚠️ **imported 9/15 — not yet eyeballed** | Imported into staging 9/15 for the same scene. **Only 4 objects in the bag**, so expect a tree and little else rather than a map. Cleanest import so far: 1 dead of 6 URLs, nothing removed, and it **fitted its own floor plate** (vBase 22.0). imgur floor image, live. New SBx `b739d8`. |
| **Low Hill** | 6e9f87 | ⚠️ **imported 9/15 — half of it pruned away** | Imported into staging 9/15 for the same scene. **30 of its 59 pieces removed** (8 dead of 16 URLs), so it may come out sparse. Its Google-Sites floor image was **dead**, and the map has no plate of its own, so the SBx paint was **swapped to Small town's grass image** (`steamusercontent-a.akamaihd.net/ugc/11935844772624656728/4538301478A9CAAB48F9148595A7F0FBAFCED99A/`, live, 800 KB) — that image has a pond in it, so swap again if the pond lands somewhere awkward. 47 GUID collisions remapped on import. New SBx `401161`. |
