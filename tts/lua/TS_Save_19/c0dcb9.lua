...
--Chooses what color dice that are rolled are. Options:are
...
function onLoad(saved_data)
    ...
    Wait.frames(function() spawnRollButtons()  end, 10)
    currentDice = {}

    self.createButton({
      click_function = "rollPercentage",
      function_owner = self,
      label = "Roll\n(Prob)",
      position = {
        -3.55, 0.05, 0
...
function click_roll(color, dieIndex, altClick)
    ...
    elseif rollInProgress ~= true then
        ...
        local newScale = (-0.01 * diceCount) + 1
...
function rollPercentage(obj, color)
    local random_number = math.random(1, 100)
    local message = Player[color].steam_name .. "     |     " .. random_number .. "%"
    Wait.time(
      function() 
...
function getPositionInLine(i, color)
    local diceInRow = maxMod(#currentDice, 8.0)
    local offsetMult = math.floor((i - 1) / 8.0)
    local locali = maxMod(i, 8.0)
    if (#currentDice > i + (8 - locali)) then
        diceInRow = 8
    end

    local widthStep = (widthMaximum / (diceInRow + 1))
    local x = (-widthStep * locali + (widthMaximum / 2)) - 0.9
    local y = heightOffset
    local z = -distanceOffset + (-distanceOffset * offsetMult) *0.8 -- (-2) puts dice in center
