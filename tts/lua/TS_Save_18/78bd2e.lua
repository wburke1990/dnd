...
function onSave()
    saved_data = {tt=titleText, ht=headerText, nt=noteText, dt=displayText}
...
function headerEndEdit(player, value, id)
    headerText = value
    save_data = {ht=headerText}

    -- This function is called every time the text in the headerText changes.
    self.UI.setValue("headerText", "text", headerText)
...
function LockUnlock(player, isOn)
    if isOn == "False" then
        ...
        self.UI.setAttribute("InputId", "position", "220 0 -80")
