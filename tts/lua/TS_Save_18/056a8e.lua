...
defaultButtonData = {
    ...
    --Saving Throws
        --STR Saving Throw checkbox
            checkboxstrst= {
                {   pos   = {-0.987,0.12,-0.343},
                    size  = 250,
                    state = false }, },
        --DEX Saving Throw checkbox
            checkboxdexst= {
                {   pos   = {-0.987,0.12,-0.246},
                    size  = 250,
                    state = false }, },
        ...
        --DEX Score counter
            dexcounterscore = {
                {   pos                = {-0.726,0.1,-0.473},
                    ...
                    value              = 10,
                    hideBG             = true,
                    acrobaticsPos      = {-0.267,0.12,-0.09},
                    sleightOfHandPos   = {-0.267,0.12,0.795},
                    stealtPos          = {-0.267,0.12,0.854} }, },
        --CON Score counter
            concounterscore = {
                {   pos    = {-0.512,0.1,-0.473},
                    size   = 200,
        ...
        --INT Score counter
            intcounterscore = {
                {   pos               = {-0.3,0.1,-0.473},
                    ...
                    value             = 10,
                    hideBG            = true,
                    arcanaPos         = {-0.267,0.12,0.028},
                    historyPos        = {-0.267,0.12,0.205},
                    investigationPos  = {-0.267,0.12,0.382},
                    naturePos         = {-0.267,0.12,0.500},
                    religionPos       = {-0.267,0.12,0.736} }, },
        --WIS Score counter
            wiscounterscore = {
                {   pos               = {-0.087,0.1,-0.473},
                    size              = 200,
                    value             = 10,
                    hideBG            = true,
                    animalHandlingPos = {-0.267,0.12,-0.031},
                    insightPos        = {-0.267,0.12,0.264},
                    medicinePos       = {-0.267,0.12,0.441},
                    perceptionPos     = {-0.267,0.12,0.559},
                    survivalPos       = {-0.267,0.1,0.911} }, },
        --CHA Score counter
            chacounterscore = {
                {   pos    = {0.128,0.1,-0.473},
                    size   = 200,
                    value  = 10,
                    hideBG = true,
                    deceptionPos    = {-0.267,0.12,0.146},
                    intimidationPos = {-0.267,0.12,0.323},
                    performancePos  = {-0.267,0.12,0.618},
                    persuasionPos   = {-0.267,0.12,0.677} }, },
        --Level XP counter
            xpcounter = {
                {   pos    = {-0.168,0.1,-0.935},
                    size   = 260,
    ...
    --Consumable Textboxes
        numbox = {
            --Maximum Hit Points (Max HP) Index: 23
                {   pos       = {0.391,0.1,-0.871}, 
                    rows      = 1,
                    width     = 360,
                    font_size = 200,
...
--============================
                ...
                if ref_buttonData.checkboxlock[1].state then
                    ...
                    local saveProf = ref_buttonData.counterprof[tableIndex].value
                        if ref_buttonData.checkboxstrst[tableIndex].state == false then
                            ...
                            self.editButton({index=11, label=(saveMod + saveProf)})
                        elseif ref_buttonData.checkboxstrst[tableIndex].state == true then
                            ...
                            self.editButton({index=11, label=(saveMod)})
                        end
                    updateSave()
                end
            end
    ...
    --============================
        --Applies value to Basic Counter display
            function click_counter(tableIndex, buttonIndex, amount)
                if ref_buttonData.counter[tableIndex].color == buttonColorBlue or ref_buttonData.counter[tableIndex].color == buttonColorRed then
                  ref_buttonData.counter[tableIndex].value = ref_buttonData.counter[tableIndex].value + amount
                    self.editButton({index=buttonIndex, label=ref_buttonData.counter[tableIndex].value})
                    updateSave()
                elseif ref_buttonData.checkboxlock[1].state then
                    ref_buttonData.counter[tableIndex].value = ref_buttonData.counter[tableIndex].value + amount
                    self.editButton({index=buttonIndex, label=ref_buttonData.counter[tableIndex].value})
                    updateSave()
                end
        ...
        --Strength Ability Counter
            function click_strcounterscore(tableIndex, buttonIndex, buttonIndex2, buttonIndex3, buttonIndex4, amount)
                if ref_buttonData.checkboxlock[1].state then
                    ...
                    --Strength Saving Throw Score
                        if ref_buttonData.checkboxstrst[tableIndex].state == true then
                            self.editButton({index=buttonIndex3, label=(saveMod + saveProf)})
                        elseif ref_buttonData.checkboxstrst[tableIndex].state == false then
                            self.editButton({index=buttonIndex3, label=(saveMod)})
                        end
    ...
    --Button for Long Rests
        function click_longrest(tableIndex, color)
            ...
            for i, num in ipairs(ref_buttonData.numbox2) do
                ...
                self.editInput({index=i+25, value=ref_buttonData.numbox2[i].value})
            end

            --Sets Temporary Hit Points to 0
            local val = ref_buttonData.numbox[2].value * -1
            click_numbox(2, color, 0, false, val)
    ...
    --Updates saved value for numbox (numbers only)
        function click_numbox(i, color, value, selected, amount)
            --If in the Unlocked state or index should be updated while locked
            if ref_buttonData.checkboxlock[1].state or i > 1 then
                if amount ~= 0 then
                    if amount == -1 and tonumber(ref_buttonData.numbox[i].value) < 1 then
                          if color == nil then
                            --Prevent no player found error message
                            Wait.time(function() self.editInput({index=i+23,value=ref_buttonData.numbox[i].value}) end,0.1)
                        else
                            broadcastToColor("Value can't be negative.", color, "Red")
                            self.editInput({index=i+23,value=ref_buttonData.numbox[i].value})
                        end
                    else
            ...
            if selected == false then
                if tonumber(value) ~= nil and (tonumber(value) < 1000 and tonumber(value) > -1) then
                    if tonumber(ref_buttonData.numbox2[i].value) > tonumber(ref_buttonData.numbox[i].value) then
                        --broadcastToColor("Value can't be more than Total.", color, "Red")
                        --Wait.time(function() self.editInput({index=i+23,value=ref_buttonData.numbox[i].value}) end,0.1)
                        --ref_buttonData.numbox2[i].value = ref_buttonData.numbox[i].value
                        updateSave()
                    else
                    ...
                    broadcastToColor("Enter a valid number.", color, "Red")
                    Wait.time(function() self.editInput({index=i+25,value=ref_buttonData.numbox2[i].value}) end,0.1)
                end
            end
        end
            ...
            function createDexScoreCounter()
                ...
                --Creates Saving Throw: Index 2
                    ...
                    --Creates button and counts it
                        self.createButton({
                            label=label, click_function="click_none", function_owner=self,
                            position={-0.781,0.12,-0.247}, 
                            height=size, width=size,
                            font_size=data.size, scale=buttonScale,
                            color=buttonColorWhite, font_color=buttonColorBlack })
                    spawnedButtonCount = spawnedButtonCount + 1

                --Creates Dexterity Skill 1 Acrobatics: Index 3 GLOB IT
            ...
            function createWisScoreCounter()
                ...
                --Creates Saving Throw: Index 32
                    ...
                    --Creates button and counts it
                        self.createButton({
                            label=label, click_function="click_none", function_owner=self,
                            position={-0.497,0.12,-0.247}, 
                            height=size, width=size,
                            font_size=data.size, scale=buttonScale,
                            color=buttonColorWhite, font_color=buttonColorBlack })
                    spawnedButtonCount = spawnedButtonCount + 1

                --Creates Wisdom Skill 1 Animal Handling: Index 33
                ...
                --Creates Wisdom Skill 2 Insight: Index 34
                    ...
                    --Creates button and counts it (Passive Insight Sense): Index 26
                        self.createButton({
                            label=(label + 10), click_function="click_none", function_owner=self,
                            position={-0.956,0.12,0.209}, 
                            height=size, width=size,
                            font_size=data.size, scale=buttonScale,
                            color=buttonColorWhite, font_color=buttonColorBlack })
                    spawnedButtonCount = spawnedButtonCount + 1

                --Creates Wisdom Skill 3 Medicine: Index 36
                ...
                --Creates Wisdom Skill 4 Perception: Index 37
                    ...
                    --Creates button and counts it (Passive Perception Sense): Index 38
                        self.createButton({
                            label=(label + 10), click_function="click_none", function_owner=self,
                            position={-0.956,0.12,0.118}, 
                            height=size, width=size,
                            font_size=data.size, scale=buttonScale,
                            color=buttonColorWhite, font_color=buttonColorBlack })
                    spawnedButtonCount = spawnedButtonCount + 1

                --Creates Wisdom Skill 5 Survival: Index 39
            ...
            function createChaScoreCounter()
                ...
                --Creates Saving Throw: Index 44
                    ...
                    --Creates button and counts it
                        self.createButton({
                            label=label, click_function="click_none", function_owner=self,
                            position={-0.497,0.12,-0.149}, 
                            height=size, width=size,
                            font_size=data.size, scale=buttonScale,
                            color=buttonColorWhite, font_color=buttonColorBlack })
                    spawnedButtonCount = spawnedButtonCount + 1

                --Creates Charisma Skill 1 Deception: Index 45
                ...
                --Creates Charisma Skill 2 Intimidation: Index 46
                ...
                --Creates Charisma Skill 3 Performance: Index 5
                ...
                --Creates Charisma Skill 4 Persuassion: Index 5
            ...
            for i, data in ipairs(ref_buttonData.xpcounter) do
                ...
                --Creates Modifier: Index 52
                    ...
                    --Creates Level EXP cap
                        self.createButton({
                            label=label, click_function="click_none", function_owner=self,
                            position={0.175,0.1,-0.933}, 
                            height=size, width=size,
                            font_size=150, scale=buttonScale,
                            color=buttonColorWhite, font_color=buttonColorBlack })
                    spawnedButtonCount = spawnedButtonCount + 1

                --Sets up add 1: Index 53
